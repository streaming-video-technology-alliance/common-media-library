 ...
 */
export class tkhd extends FullBoxBase<TrackHeaderBox> {
	...
	static read(view: IsoView): TrackHeaderBox {
		...
		return new tkhd(creationTime, modificationTime, trackId, reserved1, duration, reserved2, layer, alternateGroup, volume, reserved3, matrix, width, height, version, flags)
	}
	...
	override get size(): number {
		...
		const timeSize = isVersion1 ? 8 : 4
		const durationSize = isVersion1 ? 8 : 4
		// timeSize + timeSize + 4 + 4 + durationSize + 8 + 2 + 2 + 2 + 2 + 36 + 8
		return 68 + timeSize + timeSize + durationSize
	}
}
