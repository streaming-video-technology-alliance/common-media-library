...
export class prsl extends FullBoxBase<PreselectionGroupBox> {
	...
	static read(view: IsoView): PreselectionGroupBox {
		...
		return new prsl(groupId, numEntitiesInGroup, entities, preselectionTag, selectionPriority, interleavingTag, version, flags)
	}
	...
	override get size(): number {
		// 4 + 4 + (entities.length * 4) + optional fields
		let size = 8 + (this.entities.length * 4)
