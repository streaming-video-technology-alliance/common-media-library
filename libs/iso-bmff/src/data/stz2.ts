 ...
 */
export class stz2 extends FullBoxBase<CompactSampleSizeBox> {
	...
	static read(view: IsoView): CompactSampleSizeBox {
		...
		return new stz2(version, flags, fieldSize, sampleCount, entrySize)
	}
	...
	override get size(): number {
		// 1 (fieldSize) + 3 (reserved) + 4 (sampleCount) + (entrySize.length * fieldSize bytes)
		const fieldSizeBytes = this.fieldSize === 4 ? 1 : this.fieldSize === 8 ? 1 : this.fieldSize === 16 ? 2 : 0
		return 8 + (this.entrySize.length * fieldSizeBytes)
	}
}
