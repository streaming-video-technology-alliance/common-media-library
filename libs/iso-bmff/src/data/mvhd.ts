 ...
 */
export class mvhd extends FullBoxBase<MovieHeaderBox> {
	...
	static read(view: IsoView): MovieHeaderBox {
		...
		return new mvhd(creationTime, modificationTime, timescale, duration, rate, volume, reserved1, reserved2, matrix, preDefined, nextTrackId, version, flags)
	}
	...
	override get size(): number {
		...
		const timeSize = isVersion1 ? 8 : 4
		const durationSize = isVersion1 ? 8 : 4
		// timeSize + timeSize + 4 + durationSize + 4 + 2 + 2 + 8 + 36 + 24 + 4
		return 84 + timeSize + timeSize + durationSize
	}
}
