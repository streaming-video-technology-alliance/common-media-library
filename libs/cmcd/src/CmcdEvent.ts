...
export type CmcdEvent = CmcdRequest & {
	 ...
	 * Custom Event Name
	 *
	 * Defines a custom event name. Maximum length is 64 characters. This key MUST be sent when the event type is 'ce'
	 * (custom event). This key MUST NOT be sent when the event type is any other value. A custom key-value pair MAY be used to
	 * transfer a custom value associated with this event. The names chosen SHOULD associate the custom event name with the custom
	 * key name.
	 *
	 * String
	 *
	 ...
	 * Event
	 *
	 * This key MUST only be used in Event mode. It MUST be present on all reports. The minimum recommended set of supported events
	 * are: `ps`, `e`, `t`, and `rr`.
	 *
	 * - `abs` - ad break start
	 * - `abe` - ad break end
	 ...
	 * Timestamp
	 *
	 * The timestamp at which the associated event occurred. The value is the number of milliseconds elapsed since the Unix Epoch
	 * (January 1, 1970, at 00:00:00 UTC), excluding leap seconds. When the event is a request for a media object, the time SHOULD
