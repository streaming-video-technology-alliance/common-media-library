...
import { getTimeOrigin } from './getTimeOrigin.ts'
 ...
 * Resource Timing reports zero for `responseStart` of a cross-origin resource without `Timing-Allow-Origin`.
 * `ttfb` is omitted when `responseStart` is absent, zero, or earlier than `startTime`. Without timing, `ttlb` measures the call.
 */
export function toResponseKeys(request: CmcdRequestLike, info: CmcdResponseInfo, origin: RequestOrigin | undefined): Record<string, unknown> {
	...
	if (timing && typeof timing.startTime === 'number') {
		keys['ts'] = Math.round(getTimeOrigin() + timing.startTime)
	...
	if (keys['ts'] === undefined && origin?.startedAt !== undefined) {
		keys['ts'] = origin.startedAt
	}
	if (keys['ttlb'] === undefined && origin?.startedAt !== undefined) {
