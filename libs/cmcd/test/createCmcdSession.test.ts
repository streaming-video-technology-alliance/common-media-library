...
describe('createCmcdSession', () => {
	...
	it('rejects invalid configuration with an actionable message', () => {
		...
		throws(() => createCmcdSession({ keys: ['constructor' as never] }), { message: 'CmcdSession: keys must be reserved keys or hyphenated custom keys, received constructor' })
		throws(() => createCmcdSession({ keys: ['1.example-x' as never] }), { message: 'CmcdSession: keys must be reserved keys or hyphenated custom keys, received 1.example-x' })
