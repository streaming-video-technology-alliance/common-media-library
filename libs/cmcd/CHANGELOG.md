...
- `CMCD_REQUEST_PROVENANCE` — the symbol key (backed by `Symbol.for('@svta/cml-cmcd/request-provenance')`) under which `createRequestReport()` stamps an opaque session token on every request it returns, including requests it does not decorate (request reporting disabled, or a transform cancels decoration). The member is typed optional on `CmcdRequestReport` so the type stays constructible by consumers; every request the reporter returns carries it. `recordResponseReceived()` attributes by this token first, so attribution survives a cancelled decoration and resolves the exact session when a `sid` string is reused (each `sid` change is a new retained session). A request carrying no token, or one written by another reporter instance, falls back to the `sid` stored in its CMCD data, then to a per-call `data.sid`, then to the current session; a token or `sid` that names no retained session drops the response rather than relabeling it. Spread and `Object.assign` carry the token through request clones; `JSON.stringify` and structured clone drop symbol keys, so read the token before such a boundary and restore it verbatim afterward ([#416](https://github.com/streaming-video-technology-alliance/common-media-library/pull/416))
