...
- `nor` is the shortest relative path when the request URL and `baseUrl` share two or more leading path segments. `urlToRelativePath` in `@svta/cml-utils` skipped shared segments. A `baseUrl` directory with two segments, such as `/v/1080p/`, produced `nor=("../1080p/seg-2.m4s")` for a segment in the same directory. The corrected value is `nor=("seg-2.m4s")`. This is a wire output change. A receiver that compares `nor` strings sees the new value. When the request URL and `baseUrl` diverged and matched again at a deeper segment, the old value resolved to a wrong URL. The fix applies to `encodeCmcd`, `toCmcdHeaders`, `toCmcdQuery`, and `CmcdReporter` when `baseUrl` is set
