...
- `CMCD_REQUEST_PROVENANCE` — the symbol key (backed by `Symbol.for('@svta/cml-cmcd/request-provenance')`) under which `createRequestReport()` stamps a frozen session-provenance record (`CmcdRequestProvenance`, carrying the opaque `token` that names the issuing session; a record rather than a bare string so future provenance members are additive for bridging players) on every request it returns, including requests it does not decorate (request reporting disabled, or a transform cancels decoration). The member is typed optional on `CmcdRequestReport` so the type stays constructible by consumers; every request the reporter returns carries it. `recordResponseReceived()` attributes by the record's token first, so attribution survives a cancelled decoration and resolves the exact session when a `sid` string is reused (each `sid` change is a new retained session). A request carrying no provenance, or a value written by another reporter instance, falls back to the `sid` stored in its CMCD data, then to a per-call `data.sid`, then to the current session; a token or `sid` that names no retained session drops the response rather than relabeling it. Spread and `Object.assign` carry the record through request clones; `JSON.stringify` and structured clone drop symbol keys, so read the value before such a boundary and restore it afterward — the record itself survives JSON, and attribution is by token value rather than object identity, so a revived copy attributes exactly ([#416](https://github.com/streaming-video-technology-alliance/common-media-library/pull/416))
