import { parseIntegerOrDecimal } from '@svta/cml-structured-field-values/parse/parseIntegerOrDecimal';
...
test('parseIntegerOrDecimal', () => {
	...
	assert.deepStrictEqual(parseIntegerOrDecimal(`-42`), { value: -42, src: `` });
	assert.deepStrictEqual(parseIntegerOrDecimal(`4.2`), { value: 4.2, src: `` });
	assert.deepStrictEqual(parseIntegerOrDecimal(`4a`), { value: 4, src: `a` });
	assert.deepStrictEqual(parseIntegerOrDecimal(`4.5`), { value: 4.5, src: `` });
	assert.deepStrictEqual(parseIntegerOrDecimal(`-4.5`), { value: -4.5, src: `` });
	assert.deepStrictEqual(parseIntegerOrDecimal(`4.0`), { value: 4, src: `` });
