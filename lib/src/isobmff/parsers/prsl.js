...
ISOBox.prototype._boxProcessors['prsl'] = function () {
	this._procFullBox();
	this._procField('group_id', 'uint', 4);
	this._procField('num_entities_in_group', 'uint', 4);
	this._procEntries('entities', this.num_entities_in_group, function (entry) {
		this._procEntryField(entry, 'entity_id', 'uint', 4);
	});
	if (this.flags & 0x1000) {
		this._procField('preselection_tag', 'utf8string');
	}
	if (this.flags & 0x2000) {
		this._procField('selection_priority', 'uint', 1);
	}
