...
- `child.stop(true)`: returns the active role, like `stop()`, and does not flush. A child's recorded events are never lost on teardown, because they already live in the root's queues, and they drain on batch fill or the root's own flush. hls.js's destroy path calls `stop(true)` at every asset boundary, so flushing there would send partly filled batches once per creative.
- `child.flush()`: delegates to the session-wide flush; the deliberate "send now" escape hatch for code that holds only a child.
...
// both events, so no public accessor is needed. The wiring is correct in
// both append modes: under `interstitialAppendInPlace` hls.js transfers the
// media element and its MediaSource to the child Hls instance and the
// parent's CMCDController detaches, so the child is the one presenting.
...
One new type-only file, `CmcdChildReporterConfig.ts`, exported from the index via `export type *`, two methods on `CmcdReporter`, and one new optional member each on `CmcdRequestReportConfig` and `CmcdRequestProvenance`. Expected `cml-cmcd.api.md` diff: the `CmcdChildReporterConfig` type, `CmcdReporter.createChildReporter()`, `CmcdReporter.activate()`, `CmcdRequestReportConfig.target` (see [Request-mode sequence numbers](#request-mode-sequence-numbers)), and `CmcdRequestProvenance.pid` (see [Late responses and retained sessions](#late-responses-and-retained-sessions)).
...
| Base provenance record | Per player | The record's `cid` is the playback's, so each playback mints its own; a child's record carries its `pid`. Re-minted on the playback's `cid` change and on session rotation. See [Late responses and retained sessions](#late-responses-and-retained-sessions). |
| Archived data snapshots | Session, keyed by playback id | A late response reads the issuing playback's rotation-time store from the session that issued it. The root archives eagerly at rotation; a child archives lazily on its first post-rotation call. |
...
A direction is recorded in [Unresolved questions](#unresolved-questions), item 4, pending a design pass: derive `bsa`/`bsda`/`bsd` automatically from the play-state transitions the reporter already observes. The spec defines a starvation event as the state changing to rebuffering, which is exactly what per-player dedup detects, so session totals become correct by construction and this section's "cannot aggregate" constraint stops applying to those three keys; `dfa` remains application-supplied either way.
...
Each reporter dedups `sta`/`pr`/`cid`/`br` transitions against its own baseline, so two players never suppress each other's transitions. (`bg` is the exception: only the root writes it, it dedups against a session-level baseline, and a root `sid` change clears that baseline together with the per-player ones.) The accepted consequence: the session stream may contain adjacent state events with the same value from *different* players. For example, the primary emits `ps` with `sta: 'p'`, then the ad child starts playing and also emits `ps` with `sta: 'p'`. Each report carries its own `cid`, so collectors can attribute both correctly. Per-player scope is resolved (see Unresolved questions, item 1): the spec's change-triggered event definitions are player-scoped, and its multi-player guidance expects per-player events under one `sid`. The implementation keeps the baselines movable to the session without a public API change.
...
### Late responses and retained sessions

The session-retention design ([rfc/cmcd-session-retention.md](cmcd-session-retention.md), accepted after this RFC's v4) attributes a `RESPONSE_RECEIVED` event by the provenance record `createRequestReport()` stamps on every returned request: the record's `sid` names a retained session or the event drops, the record's issue-time `cid` and encoded per-call data carry the request-time inputs, and the retained session's archived store supplies the rest. That design predates children and archives one store per session. Children make the store per playback, so "the session's data as it stood at the transition" stops naming one thing: a late response to an ad segment must not report the primary content's buffer state.

This revision resolves the composition:

- `CmcdRequestProvenance` gains one optional member, `readonly pid?: string`: the reporter-internal identity of the playback that issued the request. A child's base provenance record is `{ sid, cid?, pid }`. Per-playback records fall out of the existing design at no cost, because the record's `cid` member is already playback-scoped; each record is re-minted on that playback's `cid` change and on session rotation. The root keeps minting records without a `pid`, so every record a single-reporter app sees today stays byte-identical, and hand-built `{ sid }` records stay honored forever.
- Attribution is unchanged. The record's `sid` resolves a retained session or the response drops, exactly as the retention design states. A `pid` never gates attribution; it only selects which archived store a resolved response reads.
- Rotation archives per playback. The outgoing session holds a map of frozen data snapshots keyed by playback id. The root's snapshot is written eagerly at rotation. A child archives lazily: the same epoch check that clears its dedup baseline on its first call after a rotation also writes its pre-rotation store into the outgoing session's snapshot map, which retention still holds. Eager enumeration is impossible by design (the root keeps no child registry), and lazy archival needs no references and rides an integer compare the child pays anyway.
- Resolution: a response attributed to an archived session reads the snapshot under the record's `pid`, falling back to the root's snapshot when the record has no `pid` or that playback never archived one (a child that made no call between the rotation and the response). A response attributed to the current session reads the recording reporter's live store, which is the issuing playback's store in the natural integration where each player records its own responses.

The degradations are graceful and bounded, in the retention design's spirit. A missing, foreign, or never-archived `pid` degrades the snapshot source to the root's store, never to a drop, and the members that carry a response's request-time meaning (the record's issue-time `cid` and its per-call data snapshot) ride the record itself, untouched by which store backfills the rest. The `pid` is a plain string member, so the record stays JSON-safe and the documented serialization bridge is unchanged. It never reaches the wire, because no encode path reads the record.

Most of the runtime half of this design already exists. The reporter architecture refactor (see [Internal implementation shape](#internal-implementation-shape)) landed per-playback base records, the per-playback snapshot map, and the epoch check, exercised today entirely through the root. Its resolution reads only the root snapshot (`snapshots.get(CMCD_ROOT_PID)`) and falls back to the live store for the current session. This feature's implementation adds the typed member, child minting, lazy archival, and the `pid` lookup with root-snapshot fallback.
...
| `stop(true)` | Flush + disarm | Returns the active role; does not flush. Events stay safely queued in the session; call `flush()` to force delivery |
| `flush()` | Flushes all queues | Delegates to the root (the queues cannot be split) |
| `update()` / `recordEvent()` / `recordResponseReceived()` | As today | Child-local store and baseline (`msd` writes through to the session; `sid` and `bg` are ignored, and per-call `sid`/`bg` are overwritten by the session stamp); enqueues into the root's queues. A late response resolves per [Late responses and retained sessions](#late-responses-and-retained-sessions) |
...
**Status (2026-08-15): all six landed in [PR #416](https://github.com/streaming-video-technology-alliance/common-media-library/pull/416), merged 2026-08-14, together with the session-retention design this RFC now composes with (see [Late responses and retained sessions](#late-responses-and-retained-sessions)). The internal partition this feature builds on landed with the reporter architecture refactor (`plans/cmcd-reporter-architecture/architecture.md`).** The table below is kept as the design record of why each fix was a prerequisite.
...
The `msd` first-write-wins change itself also alters root behavior: today `update({ msd })` can be revised at any time and is captured per target at enqueue, so a root-only app that revises `msd` before its first report would lose the revision. That one is a consequence of this proposal rather than a pre-existing bug, so it stays here. Resolved as acceptable, with the freeze unconditional; see [Unresolved questions](#unresolved-questions), item 6.

One additional workstream is planned with this PR, pending a design pass: automatic session-level tracking of the buffer-starvation counters (`bsa`, `bsda`, `bsd`) derived from play-state transitions, replacing caller-supplied values for those keys. See [Unresolved questions](#unresolved-questions), item 4.
...
The partition this section originally sketched is no longer hypothetical: the reporter architecture refactor (`plans/cmcd-reporter-architecture/architecture.md`) landed it. Playback-owned state is one internal object (`CmcdPlaybackState`: data store, dedup baseline, base provenance record, observed epoch); session-owned state lives behind a ledger (`CmcdSessionState` and `CmcdSessionLedger`: per-target counters and gates, per-target delivery outboxes, retention, the per-playback snapshot map, the rotation epoch); report production is a pure pipeline (`applyReportPolicy`, `stampReport`, with counters committed only after encoding succeeds); and `CmcdReporter` is a facade of translation. No public `CmcdSession` type is added. A child is a second, smaller facade holding its own playback state and a reference to the shared ledger and outboxes, so a future public session object (N sibling reporters, no special root) stays purely additive. Session resets reach children through the ledger's epoch counter (an integer check on the child's next call), so the root keeps no child registry and no child references; the same check performs the child's lazy snapshot archival. References point only from child to root and from child to its creating reporter (the same edge, for a direct child), plus the one bounded active-pointer edge. A dropped child is garbage immediately. A forgotten `stop()` on an activated child keeps one small object alive; the real cost is that `TIME_INTERVAL` reports keep using that dead playback's last data until the next `activate()`. The memory is trivial, the report data is not (see Drawbacks).
...
- Lifecycle: child `start()` arms nothing (interval spies); child `stop()` never disarms session timers; child `stop(true)` sends nothing (a partial batch stays queued and drains on batch fill or the root's flush) while `child.flush()` drains the session's queues; active-role transfer including the `stop()` auto-return to the creating reporter, nested case included (an activated grandchild's `stop()` restores the still-presenting child that created it, not the root); a child `stop()`/`start()` cycle leaves the creator active until `activate()` is called again; an activated child abandoned without `stop()` keeps feeding periodic reports until the next `activate()` (assert report content); repeated `start()` stays safe with children present.
...
- Provenance: a child's decorated requests carry `{ sid, cid, pid }`; the root's records carry no `pid` and are byte-identical to today's; a session rotation and a child `cid` change each re-mint the child's record.
- Late-response resolution: a child's response landing after a `sid` change reports under the issuing session with the child's archived snapshot when the child reported after the rotation, and with the root-snapshot fallback when it did not; a record with an unknown `pid` falls back to the root snapshot and never drops; a hand-built `{ sid }` record resolves to the root snapshot, as today.
- Lazy archival: a child's first post-rotation call writes its pre-rotation store into the outgoing session exactly once, and a second rotation archives the store as it stood at the second transition.
...
- **Child `flush()` flushes the whole session's queues, and child `stop(true)` does not flush at all.** Per-child flushing would need per-item attribution in shared queues, so the explicit `flush()` is session-wide: an ad SDK forcing delivery also drains other players' accumulating batches. And because a child's `stop(true)` skips the flush (see Unresolved questions, item 8), an integrator must not assume boundary delivery happened at teardown: the child `stop()` TSDoc states it, and events drain on batch fill, interval accumulation, or the root's teardown flush.
...
- **A late child response's snapshot can degrade to the root's store.** Lazy archival means a child that never reports after a rotation never archives its store, so its late responses read the root snapshot for session-scoped enrichment keys. The record's issue-time `cid` and per-call data are unaffected, which bounds the loss. Eager archival would need the child registry this design refuses to hold.
...
- **`never`-typed locks over `Omit`-only.** `Omit<CmcdReporterConfig, 'sid' | …>` on its own only rejects extra properties on object literals; a widened object passes `sid` through silently. Explicit `never` fields also block widened objects, keep the lock list easy to audit, and show the *reason* in the editor tooltip. Verified: with the locks declared, `createChildReporter({ sid })`, `createChildReporter(widenedObjectWithSid)`, and the spread form `createChildReporter({ ...widenedObjectWithSid })` all fail to compile; `Omit` alone accepts the widened and spread forms, because excess-property checking does not inspect spread properties. The child config does use one `Omit`, to drop the inherited `version` so the `never` lock can replace it; that is a mechanical necessity, not a substitute for the locks. Cost: locked fields appear in autocomplete (with their explanation). Resolved as proposed; see Unresolved questions, item 7.
...
1. **State-change dedup scope.** Does the no-consecutive-duplicates expectation apply per player or per session stream? Per-child baselines (this proposal) can put adjacent same-valued `ps` lines from different players on one `sid` stream, distinguishable by `cid`/`ts`.
...
4. **Are `bsd` and the absolute counters in scope?** `bsd` is a once-per-mode-per-destination key, the same clause shape that justifies session-gating `msd`, but it has no gate for roots today or children here. Should it get the `msd` treatment in the prerequisite PR? And is documenting that the application owns `bsa`/`bsda`/`dfa` sufficient, or should children suppress them outright given a child cannot know a session total?
   ...
   **Direction (2026-08-11), pending a design pass: track the starvation counters automatically.** The spec defines the counting rule in terms of the state machine the reporter already observes ("a buffer starvation event occurs when the state changes to rebuffering"), and per-player state-change dedup detects exactly that transition. So `bsa` (count accepted transitions into `'r'` on the session, across all players), `bsda` (accumulate the span from entering `'r'` to the next accepted transition out), and `bsd` (queue each completed span for delivery once per mode and destination) are all derivable with no aggregation of opaque inputs: the reporter counts events it already observes, so multi-player session totals are correct by construction, and the spec's own state definition filters for free ("this state is not reported during startup or seeking"). This also retires the `msd`-vs-`bsd` inconsistency and the repetition trap together: `bsd` stops being caller-supplied, so a persisted `update({ bsd })` repeating on every report stops being possible. `dfa` is the exception: dropped frames are rendering-pipeline data the reporter cannot see, so it stays application-supplied. The design pass, planned with the prerequisite PR, covers precedence when an application also supplies values, cause-token enrichment (auto-tracking omits the cause, which the spec permits), duration fidelity (transitions are timestamped at report time), and `dfa`'s ownership.
5. **`child.update({ sid })` and `child.update({ bg })` are silently ignored.** This conflicts with the repo's preference for actionable failures, but throwing breaks controller code that forwards whole data objects. Is a dev-mode console warning worth it?
   ...
   **Resolution (2026-08-11): no warning; the silent ignore stands.** The question presupposed a mechanism the repo does not have: there is no dev/prod build split of any kind (no `NODE_ENV`, `__DEV__`, or `import.meta.env` anywhere; a bare unconfigured tsdown build; one `dist/index.js` for every export condition), so a "dev-mode" warning would ship unconditionally to production bundles, and class methods are not tree-shakeable, so every importer would pay for it on every playback. Silent tolerance is also the reporter's verified convention rather than an anomaly: invalid event targets are dropped at construction, `update()` strips `msd` from every payload, state events missing their required field are dropped, and the root itself silently no-ops `update({ sid })` when the value is unchanged, which is the common forwarded-whole-object case. A child warning would fire on input the root accepts silently, breaking root/child substitutability, unless further gated on value-differs plus once-per-child dedup, at more shipped cost, and the RFC's own hls.js sketch never calls `update({ sid })` on a child, so the warning would emit nothing where it matters. The signal lives in the designated channels instead: the compile-time `never` locks with explanatory tooltips, the tree-shakeable `validateCmcd*` diagnostics layer, the wire itself (child reports visibly carry the root's `sid`), and the planned no-effect tests. One obligation follows: the `update()` TSDoc must document that session-owned keys are ignored on a child. If a runtime signal is ever wanted, the convention-compatible home is an opt-in diagnostics hook, not a console call in the hot path.
6. **`msd` first-write-wins changes root behavior.** Today `update({ msd })` can be revised at any time and is captured per target at enqueue, so a root-only app that revises `msd` before its first report would lose the revision under this proposal. Acceptable, or should the freeze apply only once a session has more than one reporter? (The six [prerequisite fixes](#prerequisite-fixes) are settled: they land in their own PR first.)

   **Resolution (2026-08-11): acceptable; the freeze is unconditional.** Today's revisability is narrower than it looks. Only a revision made before any report goes out is honored consistently, because each target captures the current value at its own first report: a later revision splits the session, one target keeping the old value while another captures the new one, emitting different `msd` values under one `sid`. First-write-wins removes that narrow capability and fixes the spec-consistency defect for root-only apps at the same time, and it matches the key's semantics: a one-shot startup measurement ("the time difference between the 'starting' and 'playing' states") that a session reset already reopens. The conditional variant was rejected because it makes `update({ msd })`'s semantics depend on whether `createChildReporter()` was ever called: the same call gives a different outcome based on distant state, the freeze point depends on child-creation timing, and the sole payoff is preserving the divergence behavior. One hazard is accepted and documented: the validation rule admits `0`, so a valid-but-wrong first write freezes for the session until a `sid` reset, where today a spurious `0` self-corrects only by accident of the falsy guard. A test pins the frozen-wrong-value behavior.
7. **Locked-field ergonomics.** Do reviewers prefer the discoverable `never`-typed locks (fields visible in autocomplete with an explanation) or a quieter `Omit`-based config that hides them but only guards object literals?
   ...
   **Resolution (2026-07-29): keep the `never`-typed locks.** Verified against the repo's compiler (TypeScript 5.8.2, strict): both variants reject `sid` in an object literal, but only the `never` locks reject it from a widened object and from a spread-built config (`{ ...widened }`), which excess-property checking never inspects and which is the dominant integration idiom, including this RFC's own hls.js sketch. The autocomplete cost is three documented entries: the language service surfaces each locked field's TSDoc as its completion documentation, which also compensates for the one cosmetic wart (under the repo's tsconfig, without `exactOptionalPropertyTypes`, tooltips and error text render the lock as `undefined` rather than `never`, so the raw error does not state the reason; the TSDoc does). The repo already ships the idiom publicly (`ExclusiveRecord` in `@svta/cml-utils` generates `never` locks for forbidden sibling keys), while its `Omit` precedents all guard fields the implementation force-overwrites, where a smuggled value is harmless by construction; here a smuggled `sid` is a false developer belief that must fail at compile time, per the project's DX priority. Compile-time regression tests pin all three smuggling idioms (literal, widened, spread) so the guarantee cannot silently regress.
8. **Child teardown flush scope.** Is a session-wide flush on `child.stop(true)` acceptable (smaller batches at every asset boundary), or should a child's flush be a no-op, given its events are already safe in the root's queues?
   ...
   **Resolution (2026-08-11): the middle path. `child.stop(true)` does not flush; `child.flush()` still delegates to the session-wide flush.** The two entry points have different callers, which the original framing missed. `stop(true)` fires mechanically at every asset boundary: hls.js's `CMCDController.destroy()` calls it unconditionally (verified in source), per creative and on error resets, so a flush there is an accident of reusing the root's destroy contract, and it silently defeats an app's configured `batchSize` at collector scale (roughly 2x to 5x request inflation during ad pods; the default `batchSize` of 1 makes the question moot for unconfigured apps). `flush()` is only ever called deliberately, and removing it too would destroy the only forced-delivery mechanism available to code that holds just a child, the ad-SDK case. Skipping the teardown flush costs nothing within a normally torn-down session: a child's events already live in the root's queues, `sn` is assigned at enqueue so late delivery never breaks ordering, and the root's own teardown flush drains everything. The spec imposes no delivery deadline and twice endorses deferred delivery (the lost-connectivity clause; `ec` buffering), while its congestion guidance recommends more aggregation per batch, not less. Two notes recorded: the page-unload window at ad boundaries is not narrowed (batching apps should wire `pagehide` to `root.flush()`, which root events already need today), and if collector feedback ever wants boundary-time delivery, the home is an explicit `childReporter.flush()` at `INTERSTITIAL_ASSET_ENDED` in the integration, not a teardown side effect.
...
- **2026-08-15 (v5)**: composes this design with the session-retention RFC, accepted after v4 was written. New section, [Late responses and retained sessions](#late-responses-and-retained-sessions): `CmcdRequestProvenance` gains an optional `pid`, children mint per-playback records while root records stay byte-identical to today's, attribution stays `sid`-only with the `pid` selecting which archived per-playback snapshot a late response reads, the root archives eagerly at rotation while children archive lazily through the same epoch check that resets their baselines, and every degraded case (no `pid`, unknown `pid`, never-archived playback) falls back to the root snapshot rather than dropping. One drawback recorded: a child that never reports after a rotation leaves its late responses reading the root snapshot for enrichment keys. Also records that the six prerequisite fixes merged in #416, and rewrites [Internal implementation shape](#internal-implementation-shape) to describe the structure the reporter architecture refactor actually landed (playback state, session ledger with per-playback snapshots, pure pipeline stages, facade), which this feature now consumes instead of builds.
- **2026-09-03 (v5 correction)**: the refactor (PR #422) landed the per-playback snapshot map and the epoch check but not the `pid`-aware resolution rule. Its data-source lookup reads the root snapshot only, so the `pid` lookup with root-snapshot fallback moves into this feature's implementation. Wording in [Late responses and retained sessions](#late-responses-and-retained-sessions) corrected to match.
