...
The runtime half of this design already exists. The reporter architecture refactor (see [Internal implementation shape](#internal-implementation-shape)) landed per-playback base records, the per-playback snapshot map, the epoch check, and the pid-with-root-fallback resolution rule, exercised today entirely through the root. This feature's implementation adds the typed member, child minting, and lazy archival.
