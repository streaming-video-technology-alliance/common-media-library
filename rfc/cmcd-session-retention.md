...
- `recordResponseReceived()` resolves the session a response belongs to by the provenance record alone, and either attributes the event to that session or drops the event. It never relabels a report with the current `sid`.
...
The value is a frozen record, `CmcdRequestProvenance`. `token` is the opaque session token. On decorated requests, `cmcd` is the request report exactly as encoded on the wire; a late `RESPONSE_RECEIVED` event rebuilds its request-time data from those bytes, so the data survives any boundary the record is carried across. The record stays opaque: read the value to carry it and restore it; never fabricate or alter one. The record is the only attribution key — a value the reporter did not write names no session it owns, so misuse drops the response instead of mis-attributing. The record shape exists for extensibility — `cmcd` itself arrived as an additive member, because bridging code copies the whole value.
...
// encoded wire bytes. One that lost its provenance is dropped: the
// record is the only attribution key.
```
...
type CmcdSession<C> = {
	sid: string
	provenance: CmcdRequestProvenance  // frozen { token }, unique per session per reporter
...
The reporter classifies by exact issued tokens, read structurally from the record: a token that names a retained session attributes, and anything else drops — evicted sessions, foreign and altered tokens, and values not shaped like a provenance record alike. The token's format is an implementation detail and the contract is opacity: a restored token must be one the reporter wrote, and misuse drops rather than mis-attributing. One base record is minted and frozen per session, decorated requests extend it with the per-request snapshot, and player code cannot alter an issued record in place.
...
Spread and `Object.assign` copy symbol-keyed properties. `JSON.stringify` and structured clone drop them, so a request that crosses such a boundary resolves by rule 3 below unless the player restores the record first.

### Attribution resolution

`recordResponseReceived()` resolves a session by the provenance record alone: the record's token names one of this reporter's retained sessions, or the event is dropped. There is no `sid`-based fallback — a record lost to a serialization boundary (and not restored), an altered or fabricated value, a request decorated by another reporter instance, a hand-built request that never passed through `createRequestReport()`, and an evicted session all name no session this reporter owns, and attributing them anywhere else would relabel them. A per-call `data.sid` is report input only, never an attribution key, and the resolved session stamps its own `sid` regardless.

Dropping means the method returns without emitting. No sequence number is consumed and no gate is touched.

An attributed event is a normal event of its session. It merges that session's frozen data snapshot with the request data decoded from the record's wire snapshot, the derived response keys, and per-call data. It is stamped with that session's `sid`, consumes that session's next per-target `sn`, can carry that session's `msd` if the target's gate is still open, and is suppressed by that session's 410 disposal state.

### The wire snapshot

Decoration already produces the report's wire bytes, so the record carries them: in query mode `cmcd` is the `CMCD` query parameter's value byte for byte, and in headers mode the same prepared dictionary is serialized once more for the record. `recordResponseReceived()` rebuilds request-time report data by decoding those bytes, with tokens revived as `SfToken`, which is lossless by construction: the reporter re-reads only what it wrote. It never re-ingests the player-facing `customData.cmcd` object, which players own, may mutate, and which loses `SfItem`/`SfToken` prototypes across the JSON bridge — shape-based revival of that object could not distinguish a revived token (`{ description }`) from anything else, so token values reached the RFC 8941 serializer as plain objects, threw at send time, and poisoned batched targets. The public object remains the request-identification view players read; the reporter never reads it at all. A snapshot that does not parse (a value the reporter did not write) contributes nothing: the event reports its derived keys over the session's data alone.
...
- Per response: one map lookup to resolve the token, plus one structured-field decode of the snapshot. Snapshot decoding puts the structured-field parser in the reporter's import graph.
...
- A token from another reporter instance drops, even when the `sid` strings match.
...
- A fabricated token value drops rather than attributing; so do a value not shaped like a provenance record, an altered token, and a hand-built request with no record.
...
- A per-call `data.sid` is never an attribution key: it neither overrides a resolved token nor rescues a request without one.
...
- **The export invites deliberate misuse.** A public symbol means a player can write under it, not only read. The contract (restore verbatim, never fabricate) is documentation, though the failure is contained: an unknown value drops, so a forged token cannot relabel another session's report.
- **Dropped events are silent.** A response outliving the retention window, carrying no record, or carrying a record this reporter did not write disappears without a signal. That follows the reporter's silent-tolerance convention, and a drop counter or debug hook is deliberately left to Future possibilities.
- **Every response must come from a decorated request.** `recordResponseReceived()` only attributes requests that passed through `createRequestReport()`, which stamps the record even when it does not decorate. A hand-built request cannot produce a `RESPONSE_RECEIVED` event.
...
- **Cross-instance topologies do not report.** A response recorded on a reporter that did not decorate the request is dropped. Only the issuing reporter can attribute its requests.
...
- **The `sid` as the provenance value (a symbol-keyed copy of it)**: fixes the cancelled-transform case, but not the other two jobs the token does. The spec calls `sid` a GUID, yet that is an unenforced caller promise about format, and the property attribution needs is freshness, not format: a syntactically perfect GUID can be reused, and per CTA-5004-B a `sid` change is a new session, never a resumption, so two uses of one `sid` are two sessions that a `sid`-keyed map cannot hold at once. Only a value the reporter mints is unique by construction, and a reporter-minted value is a token whatever it is called. A copied `sid` also cannot tell two generations of one name apart: a response from the first s1 would attribute to the retained second s1, which is exactly the relabeling this design forbids. The instance-scoped token cannot collide.
