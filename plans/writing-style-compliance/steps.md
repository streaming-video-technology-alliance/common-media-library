...
**Review round (2026-09-15):** Casey reviewed the "Noticed, not changed" list and asked for changes to every item except the `ab` spec text and the `reportingMode` name. The branch was rebased onto `main` at 4bf937bd, because #464 and #465 merged in the meantime. The review round also changed the package summary in the `index.ts` of the three packages. `check-tsdoc.sh` now reports ten expected failures against `main`:
