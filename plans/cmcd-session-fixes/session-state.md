...
- **`recordResponseReceived()`** resolves its session from an attribution key: the `sid` stored in the request's `customData.cmcd` (written by `createRequestReport()`), falling back to a per-call `data.sid`, falling back to the current session.
  - The stored value wins when both exist: it is reporter-written provenance, and a caller must not be able to override where a recorded request actually ran. The per-call fallback exists so a player can attribute responses even when `sid` is not in the request-mode `enabledKeys` (nothing stored on the request in that configuration).
  - Key matches a retained session: emit into that session. The report merges `{ ...session.data, ...storedCmcd, ...derived, ...perCallData, sid: session.sid }`, consumes that session's next per-target `sn`, and can carry that session's `msd` if its gate is still open. An archived session's report is therefore labeled, sequenced, and contextualized by its own session, and the review repro flips from `cid="c1",sid="s2",sn=0` to `cid="c1",sid="s1"` with `s1`'s next `sn`.
  - Key matches no retained session (evicted, or a caller-supplied value the reporter never owned): drop the event. The reporter cannot distinguish the two, and emitting under the current `sid` would be the relabeling bug, caller-induced. `sessionRetention` controls how far back attribution reaches.
  - No key at all: current session, unchanged.
  - Consequence: per-call `sid` on this method is an attribution key, not an ignored reserved key, which is an intentional asymmetry with `recordEvent()` (no request context, so events are always current and its per-call `sid` stays ignored). The TSDoc on both methods states this, and the reserved-key sealing test for `recordResponseReceived()` changes from "emits with the canonical sid" to "attributes or drops". The stamp still writes `session.sid`, so a per-call value never labels a report directly; it can only select among retained sessions.
...
- Attribution keys: per-call `data.sid` attributes when request mode never stored one; the stored `sid` wins over a conflicting per-call value; an unknown or evicted key drops the event; no key emits as current; `recordEvent()` still ignores per-call `sid`.
- Attribution boundaries: evicted session drops; `sessionRetention: 1` drops any stale response; retention normalization table (`0`, `-1`, `NaN`, `2.5`, `undefined`).
