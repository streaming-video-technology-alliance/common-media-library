...
- **Defect-fix ledger**: exactly five behavior changes are in scope, all open findings from the #416 final review. (1) `sn`/`msdSent` committed before encode can throw (Task 4). (2) Eviction destroying a session an active fan-out still holds (Task 3). (3) `copyItemValue` sharing `SfToken` values and `Uint8Array` params/values (Task 1). (4) `stop()` during `start()`'s fan-out leaving later targets armed (Task 6). (5) Full retained-session scan on every processing pass (Task 5, performance only, no regression test). Anything else that changes observable behavior is out of scope. Amendment after execution: two more changes shipped with changelog entries. Task 1 also fixed the bare `Date` and `Uint8Array` copy crash (commit b0a19c876), and the `bg` relocation means `update({ bg: undefined })` no longer leaves an undefined `bg` key in the store, a request-mode wire change. Amendment from the PR review, 2026-09-09: two pre-existing defects are fixed on this branch with changelog entries. The required-key restore baseline is captured before the transform's copy is made, so in-place mutation cannot reach it. `batchSize` is normalized to a positive integer, so a fractional or negative value cannot recurse the queue processor.
