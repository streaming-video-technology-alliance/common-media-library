...
Decisions from the review (see `findings.md`): plain fix PR, no RFC. The encoder drops an offending key silently. The validator reports an error for every rule, including the trailing line feed.
...
5. `validateCmcdEvents`: report a body that ends with a line feed as an error.
...
- The trailing line feed is an error in the validator. A warning was proposed for compatibility with earlier `@svta/cml-cmcd` versions, whose event reports all end with one. The error was chosen for strict conformance.
