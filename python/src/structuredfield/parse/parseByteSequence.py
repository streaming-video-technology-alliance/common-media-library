...
from ...structuredfield.utils.BYTES import BYTES
...
def parseByteSequence(src):
    if not src or src[0] != ":":
        raise parseError(src, BYTES)
    ...
    b64_content = re.search("(^.*?)(:)", src)
    src = src[b64_content.span()[1] :]
    # pass b64_content char check step 6
    return ParsedValue(base64.b64decode(b64_content.group()), src)
